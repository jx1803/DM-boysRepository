...
<form id="form1" name="form1" action="<%=path %>user/memberList.action" method="post" >
	...
	<a href="javascript:;" onclick="member_add('添加用户','<%=path%>user/memberListAdd.action','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加用户</a></span> <span class="r">共有数据：<strong>88</strong> 条</span> </div>
	<div class="mt-20">
		...
		<tbody>
			<c:forEach items="${ulist}" var="list">
				...
				<c:if test="${list.parambean.param=='禁用' }">
					...
					<a title="编辑" href="javascript:;" onclick="member_edit('编辑','<%=path %>/user/memberListUpdate.action','4','','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> 
					...
					<a title="删除" href="<%=path %>user/delUser.action?adminId=${list.adminId}" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
				</td>
			</tr>
			</c:forEach>
		</tbody>
