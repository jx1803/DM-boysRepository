...
<body>
	<nav class="breadcrumb">
		...
		库存工作 <span class="c-gray en">&gt;</span> 退还厂家审核<a
	...
	<br>
	<form id="returnApply" action="toReturnManuAuditList.action"
		method="post">
		<div class="text-c">
	...
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg">
			<thead>
				<tr>
					<th scope="col" colspan="12">退厂列表</th>
				</tr>
				<tr class="text-c">
					<th>退厂药品编号</th>
					<th>退厂药品名称</th>
					...
					<tr class="text-c">
						...
						<td>
							<button type="button" class="btn btn-success"
								onclick="pass('${returnApply.drugApplyId}',6,'${returnApply.stoDrugBean.drugId }','${returnApply.applyNum}','${returnApply.manuBatch}','${returnApply.stoDrugBean.drugmanu}','${User.adminId}','${returnApply.bdBean.purPrice}','${returnApply.bdBean.proDate}')">通过</button>

							<button type="button" class="btn btn-default"
								onclick="noPass('${returnApply.drugApplyId}',8,'${returnApply.stoDrugBean.drugId }','${returnApply.applyNum}','${returnApply.bdBean.batchDetailId}','${User.adminId}')">不通过</button>
						</td>
	...
	<script type="text/javascript">
		/* 通过 */
		function pass(aid, cid, drugId, drugNum, manuBatch, drugmanu,
				auditorId, purPrice, proDate) {
			var r = confirm("确认通过审核？");
			if (r) {
				location.href = "returnManuAudit.action?drugApplyId=" + aid
						+ "&checkId=" + cid + "&drugId=" + drugId
						+ "&applyNum=" + drugNum + "&bdBean.manuBatch="
						+ manuBatch + "&stoDrugBean.drugmanu=" + drugmanu
						+ "&auditorId=" + auditorId + "&bdBean.purPrice="
						+ purPrice + "&bdBean.proDate=" + proDate;
			}
		}
		/* 不通过 */
		function noPass(aid, cid, drugId, drugNum, batchDetailId, auditorId) {
			var r2 = confirm("确认不通过审核？");
			if (r2) {
				location.href = "returnManuAudit.action?drugApplyId=" + aid
						+ "&checkId=" + cid + "&drugId=" + drugId
						+ "&applyNum=" + drugNum + "&bdBean.batchDetailId="
						+ batchDetailId + "&auditorId=" + auditorId;
			}
		}
		...
		function prePage(pageNum) {
			...
			if (pageNum > 1) {
				pageNum -= 1;
				str1 = "toReturnManuAuditList.action?pageNum=" + pageNum;
			} else {
				return;
			}
